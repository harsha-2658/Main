...
if st.session_state.get("file_id") != file_id:
    with st.spinner("Setting up agents..."):
        ...
        st.session_state.graph = build_graph(qa_chain, calc_agent, llm_2, embeddings)
        st.session_state.file_id = file_id
        # Content-hash cache namespace (from build_rag_chain) -- used as the
        # key for the exact/semantic Q&A caches so identical CSV content
        # shares cached answers even across different uploads/sessions.
        st.session_state.cache_id = qa_chain["file_id"]
...
if question:
    ...
    with st.chat_message("assistant"):
            ...
            result = st.session_state.graph.invoke(
                {
                    ...
                    "route": "",
                    "answer": "",
                    "file_id": st.session_state.cache_id,
                    "cache_hit": "",
                }
            )
        ...
        cache_labels = {
            ...
            "none": "🤖 live agent call",
        }
        caption = f"{cache_labels.get(cache_hit, cache_hit)}  |  routed to: {route} agent"
